...
import 'package:kongde/widgets/lyrics_view.dart';
...
class PlayOnlineMusicPage extends StatefulWidget {
  const PlayOnlineMusicPage({super.key});

  @override
  State<PlayOnlineMusicPage> createState() => _PlayOnlineMusicPageState();
}

class _PlayOnlineMusicPageState extends State<PlayOnlineMusicPage> {
  ...
  Future<void> _loadLyrics(String songId) async {
    try {
      ...
      final response = await getSongLyricsForDart(songId: songUuid);
      // 优先取 LRC 格式
      final lrcText = response.lrc;
            ...
            child: StreamBuilder<MediaState>(
              ...
              builder: (context, snapshot) {
                ...
                return LyricsView(
                  lines: _lyricsLines,
                  position: position,
                  textColor: iconColor.withValues(alpha: 0.5),
