...
pub fn extract_colors(image_bytes: &[u8]) -> (Option<i64>, Option<i64>) {
    ...
    for &color in counts.keys() {
        ...
        for (&oc, &cnt) in &counts {
            ...
            if color_dist(color, oc) < 30.0 {
                total += cnt as u32;
                sr += oc.0 as u32 * cnt as u32;
                sg += oc.1 as u32 * cnt as u32;
                sb += oc.2 as u32 * cnt as u32;
                done.insert(oc);
            }
        }
        if total > 0 {
    ...
    let filtered: Vec<_> = sorted
        .into_iter()
        .filter(|(c, _)| !is_dark(c.0, c.1, c.2))
        .collect();
    if filtered.is_empty() {
        return (None, Some(0xFF2196F3));
    }
    ...
    let mut sec_best = None;
    let mut sec_ct = 0.0f64;
