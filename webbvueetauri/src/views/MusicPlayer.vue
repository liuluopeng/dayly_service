...
function setupSpectrum() {
  try {
    ...
    // createMediaElementSource 失败 = 元素已被全局连接过，旧链路仍有效
...
function startSpectrum() {
  ...
  // 对数频率: bar i → 频率 = 20 * (20000/20)^(i/(N-1))
    ...
    <div class="relative z-10 flex-1 flex items-center justify-center px-8">
      <div class="flex items-center gap-10 max-w-4xl w-full">
        ...
        <div class="flex-1 min-w-0">
          <div class="mb-6">
            <h2 class="text-3xl font-bold text-white mb-1 truncate">{{ song?.title || t('musicPlayer.unknownSong') }}</h2>
            <p class="text-white/50 text-lg">{{ song?.artist || t('musicPlayer.unknownArtist') }} - {{ song?.album || t('musicPlayer.unknownAlbum') }}</p>
          </div>
          ...
          <div ref="lyricsContainerRef"
            ...
            <p v-if="!parsedLyrics.length" class="text-white/20 text-center leading-[9rem]">{{ t('musicPlayer.noLyrics') }}</p>
