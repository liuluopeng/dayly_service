...
import { search_collins } from "../types/wasm-typed";
...
async function searchDict() {
  ...
  searchError.value = "";
  try {
    const result = await search_collins(word.value);
    searchResult.value = result;
    console.log('返回结果:', result);
    console.log('结果类型:', typeof result);
    console.log('是否为字符串:', typeof result === 'string');
  } catch (error) {
    console.error("柯林斯词典搜索失败:", error);
...
<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-center text-blue-600 mb-8">{{ t('collinsSearch.title') }}</h1>

    <div class="bg-white p-6 rounded-lg shadow-md">
      <h2 class="text-xl font-semibold mb-4">{{ t('collinsSearch.title') }}</h2>
      <p class="mb-4">使用Rust WASM API搜索柯林斯词典</p>

      <div class="mb-4">
        <input ref="searchInput" v-model="word" :placeholder="t('collinsSearch.placeholder')" class="px-4 py-2 border border-gray-300 rounded mr-2"
          @keyup.enter="searchDict" />
        <button @click="searchDict" :disabled="searchLoading" class="px-4 py-2 bg-blue-500 text-white rounded">
          {{ searchLoading ? t('common.loading') : t('collinsSearch.search') }}
        </button>
      </div>
