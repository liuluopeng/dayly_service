...
pub async fn ocr_image(mut multipart: Multipart) -> ApiResult<Json<serde_json::Value>> {
    ...
    let data = image_data.ok_or_else(|| ApiError::BadRequest {
        code: "NO_IMAGE".into(),
        message: "未找到图片，请使用 field name 'image' 上传".into(),
    })?;

    // 用 image crate 解码为 RGBA
    let img = image::load_from_memory(&data).map_err(|e| ApiError::BadRequest {
        code: "IMAGE_DECODE_ERROR".into(),
    ...
    let text =
        tokio::task::spawn_blocking(move || common::ocr::detect_and_recognize(&pixels, w, h))
            .await
            .map_err(|e| {
